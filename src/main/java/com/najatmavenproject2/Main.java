...
public static void insertValuesInDbApi() throws IOException, InterruptedException { 
	...
	//	Gson gson=new Gson(); //print all data in one line then insert
		...
		for (Data x : data) {    
			...
			String SqlQuery =
					"INSERT INTO ApiData (common,official,cca2,ccn3,cioc,independent,status,unMember,name,symbol,root,region,subregion,eng,landlocked,area,f,m,flag,googleMaps,openStreetMaps,population,fifa,side,png,svg,startOfWeek) VALUES"
					+ " ('" + common + "' ,'" + official + "', '" + cca2+"','" + ccn3 
			         + "' ,'" + cioc + "'," + independent+",'" + status + "' , " + unMember+ " ,'" 
					  + name + "', '" + symbol+"','" + root + "','" + region 
			         + "' ,'" + subregion + "', '" + eng +"'," + landlocked + "," + area + ",'"
					  + f + "', '" + m +"','" + flag + "','" + googleMaps +"','"
			          + openStreetMaps + "', " + population+ " ,'" + fifa + "' ,'" + side +  "','"
			         + png + "', '" + svg +"','" + startOfWeek +"')";
		 ...
		 for (Data x : data) {    //for-each
			 String common = x.getName().getCommon();
				...
				System.out.println("common:" + common +", official: " + official +", cca2:" + cca2 + ", ccn3:" + ccn3
						+ ", cioc:" + cioc + " "+ ", independent:" + independent + ", status:"+ status +
						",unMember:"+ unMember + ", name:"+ name + ", symbol:"+ symbol +", root:"+ root+ 
						", region:"+region+ ", subregion:"+subregion+", eng:"+eng+", landlocked:"+landlocked
						+", area:"+ area+ ", f:"+f+", m:"+", flag:"+flag+", googleMaps:"+googleMaps+
						", openStreetMaps:"+openStreetMaps+", population:"+population+", fifa:"+ fifa +
						", side:"+side+", png:"+png+", svg:"+svg+", startOfWeek:"+startOfWeek);
		 }
		 }catch (Exception ex) {
				System.err.println(ex);
			}
	    ...
	    try {
			 ...
			 System.out.println("Enter the id of the row to update:");
				...
				if (rs.next()) {
					...
					System.out.println("common:"+commonn + ", official:" + officiall + ", cca2:" + cca22 +
					", ccn3:" + ccn33 + ", cioc:" + ciocc + ", independent:" +independent+ ", status:" + status
					+", unMember:"+unMember+", name:"+name+", symbol:"+symbol+", root:"+root+", region:"+region
					+", subregion:"+subregion+", eng:"+eng+", landlocked:"+landlocked+", landlocked:"+landlocked
					+", area:"+area+", f:"+f+", m:"+m+", flag:"+flag+", googleMaps:"+googleMaps
					+", openStreetMaps:"+openStreetMaps+", population:"+population+", fifa:"+fifa
					+", side:"+side+", png:"+png+", svg:"+svg+", startOfWeek:"+startOfWeek);
				}
			} catch (Exception e) {
